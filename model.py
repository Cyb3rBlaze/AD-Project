...
def down_sample(input_layer, num_filters, kernel_size, strides, padding, max_pool):
    ...
    intermediate_layer = tf.keras.layers.Conv3D(num_filters, kernel_size=kernel_size, strides=strides, padding=padding, activity_regularizer=l1)(input_layer)
...
def up_sample(input_layer, num_filters, kernel_size, strides, padding, max_pool):
    ...
    intermediate_layer = tf.keras.layers.Conv3D(num_filters, kernel_size=kernel_size, strides=strides, padding=padding)(intermediate_layer)
    intermediate_layer = tf.keras.layers.BatchNormalization()(intermediate_layer)
    intermediate_layer = tf.keras.layers.LeakyReLU()(intermediate_layer)
...
def create_autoencoder(input_dims, num_filters, kernel_size, strides, padding, max_pool):
    ...
    final_layer = tf.keras.layers.Conv2D(1, kernel_size[0], strides=1, padding=padding[0], activation="sigmoid")(intermediate_layer)

    return final_layer


def create_encoder(input_layer, num_filters, kernel_size, strides, padding, max_pool):
    intermediate_layer = input_layer
...
def create_decoder(inputs, num_filters, kernel_size, strides, padding, max_pool):
...
def create_av45_fdg_comparison_model(input_dims, num_filters, kernel_size, strides, padding, max_pool):
    ...
    #AD
    encoder_ad_av45 = create_encoder(ad_input_av45, num_filters, kernel_size, strides, padding, max_pool)
    decoder_ad_av45 = create_decoder(encoder_ad_av45, num_filters, kernel_size, strides, padding, max_pool)

    encoder_ad_fdg = create_encoder(ad_input_fdg, num_filters, kernel_size, strides, padding, max_pool)
    decoder_ad_fdg = create_decoder(encoder_ad_fdg, num_filters, kernel_size, strides, padding, max_pool)

    #Unknown
    encoder_unknown_av45 = create_encoder(unknown_input_av45, num_filters, kernel_size, strides, padding, max_pool)
    decoder_unknown_av45 = create_decoder(encoder_unknown_av45, num_filters, kernel_size, strides, padding, max_pool)

    encoder_unknown_fdg = create_encoder(unknown_input_fdg, num_filters, kernel_size, strides, padding, max_pool)
    decoder_unknown_fdg = create_decoder(encoder_unknown_fdg, num_filters, kernel_size, strides, padding, max_pool)

    #CN
    encoder_cn_av45 = create_encoder(cn_input_av45, num_filters, kernel_size, strides, padding, max_pool)
    decoder_cn_av45 = create_decoder(encoder_cn_av45, num_filters, kernel_size, strides, padding, max_pool)

    encoder_cn_fdg = create_encoder(cn_input_fdg, num_filters, kernel_size, strides, padding, max_pool)
    decoder_cn_fdg = create_decoder(encoder_cn_fdg, num_filters, kernel_size, strides, padding, max_pool)
    ...
    final_difference = tf.keras.layers.Lambda(average)([final_difference_av45, final_difference_fdg])
